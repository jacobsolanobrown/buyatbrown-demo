...
export default function Furniture () {
  ...
  const clothesFilters = ["Bedding", "Beds & Mattresses", "Tables", 
  "Seating", "Dressers & Storage", "Lighting", "Decorations", "Flooring & Rugs", "Organization", "Kids", "Pets", "Other"];
  ...
  useEffect(() => {
    ...
    filterListings("ignore", "furniture", tagsString, conditionsString) // no filtering by title
        .then((data) => {
        ...
        .catch((err) => {
          setErrorMessage("Error fetching furniture listings. (Error:  " + err + ")");
          console.error("Error fetching furniture listings", err);
        })
      ...
      <div className="w-full h-full p-5 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-8">
        ...
        {isLoading ? (
          ...
          />
            ...
            <p className="align-center p-4 text-3xl font-ibm-plex-sans text-center text-red-600">No furniture listings available</p>
          ) : (
