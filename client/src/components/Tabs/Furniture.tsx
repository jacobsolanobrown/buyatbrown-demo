...
export default function Furniture() {
  ...
  useEffect(() => {
    ...
    filterListings("ignore", "furniture", tagsString, conditionsString) // no filtering by title
      ...
      .catch((err) => {
        setErrorMessage(
          "Error fetching furniture listings. (Error:  " + err + ")"
        );
      ...
      <div className="w-full h-full p-5 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-8 mx-auto">
        ...
        {isLoading ? (
          ...
          <div aria-label="message" className="flex justify-center min-w-full h-dvh p-5">
            <p className="p-4 text-3xl font-ibm-plex-sans text-center text-red-600">
              No furniture listings available with filters: {selectedFilters.join(", ") + ", " + selectedConditions.join(", ")}
            </p>
          </div>  
        ) : posts.length === 0  ? (
          <div aria-label="message" className="flex justify-center min-w-full h-dvh p-5">
            <p className="p-4 text-3xl font-ibm-plex-sans text-center text-red-600">
              No furniture listings available.
            </p>
          </div>  
        ) :
        (
