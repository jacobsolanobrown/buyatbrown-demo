...
class AddListingHandlerTest {
  ...
  void testAddListingGoodRequest() throws ExecutionException, InterruptedException, IOException {
    ...
    assertEquals("Tables,Organization,Dressers & storage,Kids,Decorations", responseMap.get("tags"));
    ...
    assertEquals("A really cool table.",
      responseMap.get("description"));
    assertTrue(responseMap.get("imageUrl").toString().contains("http://mock-storage/"));
  }
  ...
  void testInvalidTags() throws ExecutionException, InterruptedException, IOException {
    ...
    assertEquals("Each tag should be less than or equal to 3 words.", tooManyWordsInTagResponseMap.get("error"));
  ...
  void testInvalidPrice() throws IOException {
    ...
    Object notNumberResult = mockAddListingHandler.handle(notNumberMockRequest,
      mockResponse);
    Map<String, Object> notNumberResponseMap = Utils.fromMoshiJson(
      notNumberResult.toString());
    assertEquals("failure", notNumberResponseMap.get("response_type"));
    assertEquals("Invalid value for price: not a number",
      notNumberResponseMap.get("error"));

    // CHECK FOR NEGATIVE NUMBER
    queryParams.put("price", "-1.00");
    MockRequest negNumberMockRequest = new MockRequest(queryParams, body);
    Object negNumberResult = mockAddListingHandler.handle(negNumberMockRequest,
      mockResponse);
    Map<String, Object> negNumberResponseMap = Utils.fromMoshiJson(
      negNumberResult.toString());
    assertEquals("failure", negNumberResponseMap.get("response_type"));
    assertEquals("Price cannot be negative or larger than 999999999.",
      negNumberResponseMap.get("error"));
    ...
    Object largeNumberResult = mockAddListingHandler.handle(largeNumberMockRequest,
      mockResponse);
    Map<String, Object> largeNumberResponseMap = Utils.fromMoshiJson(
      largeNumberResult.toString());
    assertEquals("failure", largeNumberResponseMap.get("response_type"));
    assertEquals("Price cannot be negative or larger than 999999999.",
      largeNumberResponseMap.get("error"));
    ...
    Object decimalNumberResult = mockAddListingHandler.handle(decimalNumberMockRequest,
      mockResponse);
    Map<String, Object> decimalNumberResponseMap = Utils.fromMoshiJson(
      decimalNumberResult.toString());
    assertEquals("failure", decimalNumberResponseMap.get("response_type"));
    assertEquals("Price has more than two decimal points.",
